...
The installer creates or reuses `.venv`, installs the local `upstream/mlx-audio` package with server and TTS dependencies, writes a LaunchAgent to `~/Library/LaunchAgents/com.nuvoread.mlx-audio-server.plist`, starts it immediately, and configures it to start at login.

...
The installer creates or reuses `.venv`, installs the local `upstream/mlx-audio` package with server and TTS dependencies, installs Kokoro text-processing dependencies (`espeak-ng`, `misaki`, and `num2words`), downloads the default Kokoro TTS model and voice preset, writes a LaunchAgent to `~/Library/LaunchAgents/com.nuvoread.mlx-audio-server.plist`, starts it immediately, runs a curl-based TTS smoke test, and configures it to start at login.
